...
empty produced a PDF with content printed outside the text block — usually a table wider than the
column, an oversized figure, or text that cannot break — so it is unfinished work no matter what the
exit code says: fix what you authored at the reported lines and recompile until the field is empty,
keeping only entries you have inspected and confirmed come from the template's own files rather than
from content you wrote. Emit {COMPLETE_PROMISE} only after verifying every mandatory template
requirement and confirming that converted/main.tex exists beside a non-empty converted/main.pdf.
