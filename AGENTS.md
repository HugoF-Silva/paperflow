...
- `src/plugins/academia-perks/skills/converter/scripts/converter/tools.py` — `_OVERFULL` / `_overfull_boxes(output, limit)` / `_MAX_OVERFULL`: pulls TeX's overfull-box verdict (content printed outside the text block) out of the captured Tectonic output, deduped across TeX reruns and sorted worst-first. `_compile` returns it as the `overfull` field so a PDF that spills past the margin stops reading as success; underfull boxes stay excluded on purpose.