...
## Procedure
1. **Ensure the API key.** The program needs `OPENAI_API_KEY` in the
   environment. If a human is present and it is missing, ask for it, then
   `export OPENAI_API_KEY=...` via Bash.
2. **Gather missing inputs** (only if a human is present), one at a time:
   (a) the paper, if none is visible - ask them to upload it; (b) `soon-days` -
   how many days they tolerate until a venue opens. **Never** ask for
   `--input-dir` - locate uploads yourself.
